...
func (c *chainPassFailCalculator) CalculateScore(results *inspect.InspectionResults) (float64, error) {
	...
	if results.Indicators[inspect.IndicatorRegistryAPIAccessible] != inspect.ResultSuccess ||
		results.Indicators[inspect.IndicatorRegistryAPIENS] != inspect.ResultSuccess ||
		results.Indicators[inspect.IndicatorRegistryAPIAssignments] != inspect.ResultSuccess {
		return 0, nil
	}
